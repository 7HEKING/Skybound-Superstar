...
function set_heading(pid)
    ...
    local a = v3.new(vc.x, vc.y, vc.z)
    local b = v3.new(p.x,p.y,p.z)
    local Fp = v3.new(b.x + (v:getX() * 0.05), b.y + (v:getY() * 0.05), b.z + (v:getZ() * 0.05))
    local Fb = v3.new(Fp:getX(), Fp:getY(), Fp:getZ())
